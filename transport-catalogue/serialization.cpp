...
namespace serialization {
    ...
    transport_catalogue_serialization(const transport_catalogue::TransportCatalogue &transport_catalogue) {
    ...
    transport_catalogue::TransportCatalogue transport_catalogue_deserialization(
        ...
        transport_catalogue.SetDistanceBetweenStops(distances);
        ...
        for (const auto &bus_proto: buses_proto) {
            ...
            for (auto stop_id: bus_proto.stops()) {
                ...
                tc_bus.stops.push_back(transport_catalogue.FindStop(name));
            }
    ...
    transport_catalogue_protobuf::Color color_serialization(const transport_catalogue::svg::Color &tc_color) {
        ...
        } else if (std::holds_alternative<transport_catalogue::svg::Rgb>(tc_color)) {
            transport_catalogue::svg::Rgb rgb = std::get<transport_catalogue::svg::Rgb>(tc_color);
        ...
        } else if (std::holds_alternative<transport_catalogue::svg::Rgba>(tc_color)) {
            transport_catalogue::svg::Rgba rgba = std::get<transport_catalogue::svg::Rgba>(tc_color);
    ...
    transport_catalogue::svg::Color color_deserialization(const transport_catalogue_protobuf::Color &color_proto) {

        transport_catalogue::svg::Color color;

        if (color_proto.has_rgb()) {
            transport_catalogue::svg::Rgb rgb;
            ...
            transport_catalogue::svg::Rgba rgba;
    ...
    render_settings_serialization(const transport_catalogue::map_renderer::RenderSettings &render_settings) {
        ...
        *render_settings_proto.mutable_underlayer_color_() = std::move(
                color_serialization(render_settings.underlayer_color_));
    ...
    transport_catalogue::map_renderer::RenderSettings
    render_settings_deserialization(const transport_catalogue_protobuf::RenderSettings &render_settings_proto) {
    ...
    routing_settings_serialization(const transport_catalogue::router::RoutingSettings &routing_settings) {
    ...
    transport_catalogue::router::RoutingSettings
    routing_settings_deserialization(const transport_catalogue_protobuf::RoutingSettings &routing_settings_proto) {
    ...
    void catalogue_serialization(const transport_catalogue::TransportCatalogue &transport_catalogue,
                                 const transport_catalogue::map_renderer::RenderSettings &render_settings,
                                 const transport_catalogue::router::RoutingSettings &routing_settings,
                                 std::ostream &out) {

        transport_catalogue_protobuf::Catalogue catalogue_proto;

        transport_catalogue_protobuf::TransportCatalogue transport_catalogue_proto = transport_catalogue_serialization(
                transport_catalogue);
        transport_catalogue_protobuf::RenderSettings render_settings_proto = render_settings_serialization(
                render_settings);
        transport_catalogue_protobuf::RoutingSettings routing_settings_proto = routing_settings_serialization(
                routing_settings);
